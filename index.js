const express = require("express");
    ...
    //   Function to map movie data from the database to the API response format
    const mapMovieResult = (row, directors, genres, actors) => {
      return {
        ...
        actors: actors.filter((actor) =>
          actor.movieIds.includes(row.movieid)
        ),
      };
    };
    
    // Function to get actors with their associated movie IDs
    ...
    app.post("/users/:username/favorites/:movieId", async (req, res) => {
      try {
        ...
        const checkResult = await client.query(
          ...
          [userId, parseInt(movieId, 10)] // Ensure movieId is an integer
        );
        if (checkResult.rows.length > 0) {
          await disconnectDB(client);
          return res
